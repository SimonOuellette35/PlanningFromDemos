...
# This Differentiable Neural Dictionary implementation is from:
# https://github.com/BKHMSI/Meta-RL-TwoStep-Task/blob/master/models/dnd.py
...
class DND:
    ...
    def save_memory(self, memory_key, memory_val):
        ...
        # get data_fourrooms is necessary for gradient reason
    ...
    def get_memory(self, query_key):
        ...
        if n_memories == 0 or self.retrieval_off:
            return _empty_memory(self.memory_dim), torch.from_numpy(np.array([sys.float_info.max]))
        ...
        similarity = torch.max(similarities)
        best_memory_val = self._get_memory(similarities)
        return best_memory_val.unsqueeze(0), similarity
