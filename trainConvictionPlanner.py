from model.ConvictionPlanner import MotionPlanner
import numpy as np
import utils
import torch
...
Z_DIM=512
LR = 0.0001
model = MotionPlanner(action_space=ACTION_SPACE, device=DEVICE, dict_len=25000) # TOOD: was 10,000
model.train()
model.to(DEVICE)
...
def generateBatch(tmp_X, tmp_a, tmp_v):
  batch_X, batch_X2, batch_val, batch_actions, batch_values = list(), list(), list(), list(), list()
  ...
  for _ in range(BATCH_SIZE):
    ...
    batch_X2.append(tmp_X[episode_idx][step_idx+1])
  ...
  return torch.from_numpy(np.array(batch_X)).to(DEVICE), \
         torch.from_numpy(np.array(batch_X2)).to(DEVICE), \
         np.array(batch_val),\
...
for epoch in range(NUM_EPOCHS):
  ...
  batch_X, batch_X2, batch_val, batch_actions, batch_values = generateBatch(training_X, training_a, training_v)

  loss = model.trainSequence(batch_X, batch_X2, batch_val, batch_actions, batch_values)
